...
export class GojiBridgeWebpackPlugin extends GojiBasedWebpackPlugin {
  ...
  private async renderSubtreeBridge(compilation: webpack.compilation.Compilation, basedir: string) {
    ...
    for (let depth = 0; depth < maxDepth; depth += 1) {
      await this.renderTemplateComponentToAsset(
        compilation,
        path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `children${depth}.wxml`),
        () =>
          childrenWxml({
            maxDepth,
      ...
      await this.renderTemplateComponentToAsset(
        compilation,
        path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `components${depth}.wxml`),
        () =>
          componentWxml({
            depth,
            componentsDepth: depth + 1,
  ...
  private async renderLeafTemplate(compilation: webpack.compilation.Compilation, basedir: string) {
    ...
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `leaf-components.wxml`),
      () =>
        leafComponentWxml({
          components: components.filter(c => c.isLeaf),
        }),
  ...
  private async renderChildrenRenderComponent(
    ...
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `subtree.js`),
      () => subtreeJs(),
    );
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `subtree.json`),
      () =>
        subtreeJson({
    ...
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `subtree.wxml`),
      () => subtreeWxml(),
    );
  }
  ...
  private async renderComponentTemplate(
    ...
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `components0.wxml`),
      () =>
        componentWxml({
          depth: 0,
          componentsDepth: 0,
  ...
  private async renderChildrenTemplate(
    ...
    await this.renderTemplateComponentToAsset(
      compilation,
      path.posix.join(basedir, BRIDGE_OUTPUT_PATH, `children0.wxml`),
      () =>
        childrenWxml({
          maxDepth: Infinity,
          componentsDepth: 0,
  ...
  private async renderWrappedComponents(
    ...
    for (const component of components) {
      if (component.isWrapped) {
        await this.renderTemplateComponentToAsset(
          compilation,
          path.posix.join(basedir, BRIDGE_OUTPUT_PATH, 'components', `${component.name}.wxml`),
          () => wrappedWxml({ component }),
        );
        await this.renderTemplateComponentToAsset(
          compilation,
          path.posix.join(basedir, BRIDGE_OUTPUT_PATH, 'components', `${component.name}.json`),
          () =>
            wrappedJson({
              relativePathToBridge: '..',
              component,
        ...
        await this.renderTemplateComponentToAsset(
          compilation,
          path.posix.join(basedir, BRIDGE_OUTPUT_PATH, 'components', `${component.name}.js`),
          () =>
            wrappedJs({
              component,
            }),
